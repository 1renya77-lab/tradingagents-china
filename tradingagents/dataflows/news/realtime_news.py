...
def get_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6) -> str:
    ...
    as_of_end = datetime.strptime(curr_date, "%Y-%m-%d").replace(
        hour=23, minute=59, second=59, tzinfo=ZoneInfo(get_timezone_name())
    )

    def _parse_publish_time(raw_value):
        if raw_value is None:
            return None
        if isinstance(raw_value, datetime):
            if raw_value.tzinfo is None:
                return raw_value.replace(tzinfo=ZoneInfo(get_timezone_name()))
            return raw_value.astimezone(ZoneInfo(get_timezone_name()))
        text = str(raw_value).strip()
        if not text:
            return None
        for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%d", "%Y/%m/%d %H:%M:%S", "%Y/%m/%d %H:%M"):
            try:
                return datetime.strptime(text, fmt).replace(tzinfo=ZoneInfo(get_timezone_name()))
            except ValueError:
                continue
        return None
        ...
        try:
            ...
            if not news_df.empty:
                if '发布时间' in news_df.columns:
                    parsed_times = news_df['发布时间'].apply(_parse_publish_time)
                    news_df = news_df.loc[parsed_times.isna() | (parsed_times <= as_of_end)].copy()
                    logger.info(f"[新闻分析] 按分析截止日期过滤后剩余 {len(news_df)} 条东方财富新闻")
            ...
            if not news_df.empty:
                ...
                report += f"📅 报告生成时间: {datetime.now(ZoneInfo(get_timezone_name())).strftime('%Y-%m-%d %H:%M:%S')}\n"
                ...
                logger.warning(f"[新闻分析] 东方财富在截止日期 {curr_date} 前未获取到 {ticker} 的新闻，耗时 {time_taken:.2f} 秒，尝试使用其他新闻源")
        except Exception as e:
    ...
    try:
        ...
        if news_items and len(news_items) > 0:
            news_items = [item for item in news_items if item.publish_time <= as_of_end]
            logger.info(f"[新闻分析] 按分析截止日期过滤后聚合器剩余 {len(news_items)} 条新闻")
        ...
        try:
            ...
            if not news_df.empty:
                if '发布时间' in news_df.columns:
                    parsed_times = news_df['发布时间'].apply(_parse_publish_time)
                    news_df = news_df.loc[parsed_times.isna() | (parsed_times <= as_of_end)].copy()
                    logger.info(f"[新闻分析] 按分析截止日期过滤后剩余 {len(news_df)} 条港股东方财富新闻")
    ...
    logger.info(f"[新闻分析] 截止日期 {curr_date} 前未获取到稳定新闻，直接返回无新闻结果，不再继续请求外部备用源")
    total_time_taken = (datetime.now(ZoneInfo(get_timezone_name())) - start_total_time).total_seconds()
    return f"""
{ticker} 截止日前无可用新闻
分析日期: {curr_date}
...
说明:
1. 已对东方财富与实时聚合结果执行截止日期过滤
2. 过滤后未发现可用于 {curr_date} 当日及之前的相关新闻
3. 为避免引入未来信息或不稳定外部源，已停止继续回退抓取

总耗时: {total_time_taken:.2f}秒
"""
