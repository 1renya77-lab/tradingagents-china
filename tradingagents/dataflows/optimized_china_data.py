...
成交量: {volume}"""
    ...
    def _generate_fundamentals_report(
        ...
        if curr_date:
            try:
                analysis_dt = datetime.strptime(curr_date, "%Y-%m-%d").replace(tzinfo=ZoneInfo(get_timezone_name()))
            except ValueError:
                analysis_dt = datetime.now(ZoneInfo(get_timezone_name()))
                curr_date = analysis_dt.strftime("%Y-%m-%d")
        else:
            analysis_dt = datetime.now(ZoneInfo(get_timezone_name()))
            curr_date = analysis_dt.strftime("%Y-%m-%d")

        analysis_date_cn = analysis_dt.strftime('%Y年%m月%d日')
        report_generated_at = datetime.now(ZoneInfo(get_timezone_name())).strftime('%Y-%m-%d %H:%M:%S')
...
---
**分析日期**: {analysis_date_cn}
**生成时间**: {report_generated_at}
**数据来源**: 基础市场数据
"""
...
- **当前股价**: {current_price}
- **涨跌幅**: {change_pct}
- **分析日期**: {analysis_date_cn}{data_source_note}
