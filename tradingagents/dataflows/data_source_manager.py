...
def _load_system_config_from_app() -> Optional[dict]:
    """Best-effort app config loader for standalone bundle runs."""
    try:
        from app.core.database import get_mongo_db_sync
        db = get_mongo_db_sync()
        return db.system_configs.find_one({"is_active": True}, sort=[("version", -1)])
    except Exception as e:
        logger.debug(f"跳过 app 数据库配置读取，使用本地默认配置: {e}")
        return None
...
class DataSourceManager:
    ...
    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[ChinaDataSource]:
        ...
        try:
            config_data = _load_system_config_from_app()
    ...
    def _get_datasource_configs_from_db(self) -> dict:
        ...
        try:
            config = _load_system_config_from_app()
            if not config:
                return {}
...
def get_china_stock_data_unified(symbol: str, start_date: str, end_date: str) -> str:
    ...
    # 分析返回结果的详细信息
    if result:
        # 兼容 result 可能是 tuple/list 或 string 的情况
