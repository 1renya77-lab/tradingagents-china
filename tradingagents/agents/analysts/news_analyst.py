...
def create_news_analyst(llm, toolkit):
    ...
    def news_analyst_node(state):
        ...
        def _is_no_news_payload(text: str) -> bool:
            if not text:
                return False
            markers = (
                "截止日前无可用新闻",
        ...
        def _build_no_news_report(raw_text: str) -> str:
            report_lines = [
                f"# {ticker}（{company_name}）新闻分析报告",
                "",
                f"- 报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                f"- 分析截止日期: {current_date} 23:59:59",
                f"- 结论: 截止分析日期前无可用新闻，新闻面暂不提供新增交易驱动。",
                "",
                "## 新闻结论",
                f"{ticker} 在 {current_date} 收盘前未检索到可用于本次分析的有效新闻、公告或稳定外部新闻条目。",
                "这意味着新闻分析师当前不给出额外利好或利空判断，后续交易决策应更多依赖技术面、基本面与其他已启用分析师输出。",
                "",
                "## 时效性说明",
                "本报告严格以分析截止日期为上限过滤新闻，晚于该日期的内容不会纳入，避免未来信息泄露。",
            ]
            cleaned_raw = (raw_text or "").strip()
            if cleaned_raw:
                report_lines.extend([
                    "",
                    "## 原始检索结果",
...
请撰写详细的中文分析报告，并在报告末尾附上Markdown表格总结关键发现。"""
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "\n4. 您的回答必须基于工具返回的真实数据"
                    "\n5. 如果工具数据包含 DATA_QUALITY: ok=false，只能说明新闻数据缺失及其影响，不得编造新闻事件或来源"
                    "\n"
                    "\n🔧 工具调用格式示例："
                    "\n调用: get_stock_news_unified(stock_code='{ticker}', curr_date='{current_date}', max_news=10)"
                    "\n"
        ...
        if ('DashScope' in llm.__class__.__name__ 
            ...
            try:
                ...
                if _is_no_news_payload(pre_fetched_news):
                    report = _build_no_news_report(pre_fetched_news)
                    logger.info(f"[新闻分析师] ℹ️ 预处理命中无新闻分支，直接输出结构化无新闻报告，长度: {len(report)} 字符")
                    clean_message = AIMessage(content=report)

                    end_time = datetime.now()
                    time_taken = (end_time - start_time).total_seconds()
                    logger.info(f"[新闻分析师] 新闻分析完成（无新闻预处理模式），总耗时: {time_taken:.2f}秒")
                    return {
...
"""
                    ...
                    elif _is_no_news_payload(forced_news):
                        logger.info(f"[新闻分析师] ℹ️ 强制获取命中无新闻分支，直接生成结构化无新闻报告")
                        report = _build_no_news_report(forced_news)
                    else:
                ...
                # 有工具调用时，执行工具并将结构化新闻结果回填给模型，生成最终新闻报告
                logger.info(f"[新闻分析师] 🔄 检测到工具调用，开始执行工具并生成最终报告")
                ...
                for tool_call in result.tool_calls:
                    ...
                    if tool is None:
                        logger.warning(f"[新闻分析师] ⚠️ 未找到工具 {tool_name}，跳过")
                        continue

                    try:
                        if tool_name == "get_stock_news_unified" and "curr_date" not in tool_args:
                            tool_args["curr_date"] = current_date
                        tool_output = tool(**tool_args)
                        tool_output = tool_output if isinstance(tool_output, str) else str(tool_output)
                        logger.info(f"[新闻分析师] ✅ 工具 {tool_name} 执行成功，输出长度: {len(tool_output)} 字符")
                    except Exception as e:
                        logger.error(f"[新闻分析师] ❌ 工具 {tool_name} 执行失败: {e}")
                        tool_output = f"工具 {tool_name} 执行失败: {e}"
                ...
                if tool_messages:
                    followup_messages = list(state["messages"]) + [result] + tool_messages
                    logger.info(f"[新闻分析师] 📝 工具结果已回填，开始二次调用 LLM 生成最终新闻分析")
                    final_result = chain.invoke({"messages": followup_messages})
                    report = final_result.content if hasattr(final_result, "content") else ""
                    logger.info(f"[新闻分析师] ✅ 二次生成完成，报告长度: {len(report)} 字符")

                    raw_tool_text = "\n\n".join(
                        msg.content for msg in tool_messages if getattr(msg, "content", "")
                    )

                    if _is_no_news_payload(raw_tool_text):
                        if not report.strip() or len(report.strip()) < 80 or _is_no_news_payload(report):
                            logger.info("[新闻分析师] ℹ️ 二次生成过短或未充分展开，改用结构化无新闻报告")
                            report = _build_no_news_report(raw_tool_text)

                    if not report.strip():
                        logger.warning(f"[新闻分析师] ⚠️ 二次生成内容为空，降级使用结构化新闻原文")
                        report = "\n\n".join(msg.content for msg in tool_messages if getattr(msg, "content", ""))
                else:
                    logger.warning(f"[新闻分析师] ⚠️ 工具调用存在但未生成 ToolMessage，使用原始内容")
                    report = result.content if hasattr(result, 'content') else ""
