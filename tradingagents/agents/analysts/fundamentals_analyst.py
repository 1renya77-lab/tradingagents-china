...
def _coerce_fundamentals_tool_args(tool_call: dict, ticker: str, start_date: str, current_date: str) -> None:
    """Ensure model tool calls cannot drop the analysis date window."""
    if not isinstance(tool_call, dict) or tool_call.get("name") != "get_stock_fundamentals_unified":
        return
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        try:
            ...
            fallback_end_date = datetime.strptime(current_date, "%Y-%m-%d")
            start_date = (fallback_end_date - timedelta(days=10)).strftime("%Y-%m-%d")
        ...
        system_message = (
            ...
            "- 不允许假设任何数据"
            "- 不允许编造公司信息"
            "- 如果工具数据包含 DATA_QUALITY: ok=false，不得编造财务、估值或目标价，只能说明数据缺失及其风险"
            "- 不允许直接回答而不调用工具"
        ...
        system_prompt = (
            ...
            "   - 如果工具数据包含 DATA_QUALITY: ok=false，必须明确标注数据缺失，不得编造财务、估值或目标价"
            "   - 公司基本信息和财务数据分析"
            "   - PE、PB、PEG等估值指标分析"
            "   - 当前股价是否被低估或高估的判断"
            "   - 合理价位区间和目标价位建议"
            ...
            if current_tool_calls > 0:
                ...
                if has_tool_result:
                    ...
                    for tc in result.tool_calls:
                        _coerce_fundamentals_tool_args(tc, ticker, start_date, current_date)
