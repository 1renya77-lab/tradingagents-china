...
def create_risk_manager(llm, memory):
    def risk_manager_node(state) -> dict:
        ...
        if past_memory_str.strip():
            memory_guidance = f"""历史经验使用约束：
- 以下内容仅可作为检索到的历史案例/历史记录引用，不能擅自改写成“我们曾经犯错”或“我们过去吃过亏”，除非记录本身明确这样写。
- 如果引用这些经验，请使用“历史案例显示”“过往记录提示”“类似情形常见风险”这类客观表述。

可参考的历史记录：
{past_memory_str}"""
        else:
            memory_guidance = """历史经验使用约束：
- 当前没有可用的历史记忆记录。
- 因此禁止写“我们复盘过类似错误”“我们曾多次误判”“我们过去吃过亏”等伪造第一人称历史经验的句子。
- 如需类比，只能写成一般性的市场经验或同类案例。"""
...
4. **谨慎使用历史信息**：如果提供了明确的历史记录，只能将其作为外部历史案例或过往记录来引用；如果没有提供，则只能基于当前分析与一般市场经验推理，绝不能虚构“我们过去的错误”或“我们真实发生过的亏损”。
5. **遵守决策周期**：{horizon_instruction}
   如果是周频信号，最终风控建议必须面向未来5个交易日或直到下一次周频再平衡前的持仓风险，不要只判断明天单日波动。

交付成果：
- 明确且可操作的建议：买入、卖出或持有。
- 基于辩论和过去反思的详细推理。
- 明确说明该建议的适用周期，以及这个周期内最主要的风险触发条件。
