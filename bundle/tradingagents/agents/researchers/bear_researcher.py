...
def create_bear_researcher(llm, memory):
    def bear_node(state) -> dict:
        ...
        if is_china:
            a_share_constraint = (
                "\nA股交易约束：你不是在建议裸卖空或建立空头仓位。"
                "你的职责是从风险角度论证是否应避免买入、卖出已有持仓、减仓、止损或空仓观望。"
                "不要使用“做空”“卖空”“建立空头仓位”等不适合普通A股交易的建议。"
            )

        prompt = f"""你是一位谨慎风险研究员，负责论证不投资或降低股票 {company_name}（股票代码：{ticker}）暴露的理由。
...
- 决策周期：{horizon_instruction}
  如果是周频信号，你的谨慎/看跌论点必须论证未来5个交易日或直到下一次周频再平衡前的风险，而不是只说明明天可能下跌
- 风险和挑战：突出市场饱和、财务不稳定或宏观经济威胁等可能阻碍股票表现的因素
- 竞争劣势：强调市场地位较弱、创新下降或来自竞争对手威胁等脆弱性
- 负面指标：使用财务数据、市场趋势或最近不利消息的证据来支持你的立场
- 反驳看涨观点：用具体数据和合理推理批判性分析看涨论点，揭露弱点或过度乐观的假设；对于A股，请把结论落到卖出已有持仓、减仓、持币观望或等待更好买点，而不是做空
...
请使用这些信息提供令人信服的谨慎/看跌论点，反驳看涨声明，并参与动态辩论，展示投资该股票的风险和弱点。你还必须处理反思并从过去的经验教训和错误中学习。
...
"""
        ...
        argument = f"Cautious Risk Analyst: {response.content}"
