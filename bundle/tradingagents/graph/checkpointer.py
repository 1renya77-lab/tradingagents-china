...
def _load_sqlite_saver() -> Any:
    """Import SqliteSaver lazily so checkpoint support stays optional.

    Some LangGraph builds do not ship `langgraph.checkpoint.sqlite`. The
    TradingAgents A-share single-run path does not use checkpointing, so we
    should not fail at module import time when this optional extra is absent.
    """
    try:
        from langgraph.checkpoint.sqlite import SqliteSaver  # type: ignore
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "Checkpoint support requires a LangGraph build with "
            "`langgraph.checkpoint.sqlite.SqliteSaver`. Either install a "
            "compatible langgraph version or keep checkpoint_enabled=False."
        ) from exc
    return SqliteSaver
