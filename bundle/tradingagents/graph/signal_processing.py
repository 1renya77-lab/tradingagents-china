...
如果某些信息在报告中没有明确提及，请使用合理的默认值。""",
        ...
        try:
            response_raw = self.quick_thinking_llm.invoke(messages).content

            # 处理不同LLM返回的content格式（Anthropic返回列表，OpenAI返回字符串）
            if isinstance(response_raw, list):
                # Anthropic格式：content是content blocks列表，需要提取文本
                response = ""
                for block in response_raw:
